...
# where the recorded data is converted to float values 
...
def updated_damages(damages):
    ...
    for data in damages:
        if "M" in data:
            f = float(data.strip('M'))
            m = f * conversion['M']
            new_lst.append(m)
        elif "B" in data:
            f = float(data.strip('B'))
            b = f * conversion['B']
            new_lst.append(b)
        elif 'Damages' in data:
...
# where the keys are years and 
# the values are lists containing a dictionary for each hurricane that occurred in that year
def hurricanes_by_year(names, months, years, max_sustained_winds, areas_affected, deaths):

  dict_by_year = {}

  hurricanes = hurricane_dict(names, months, years, max_sustained_winds, areas_affected, deaths)
  ...
  for hurricane in hurricanes:
    ...
    if current_year not in dict_by_year:
      dict_by_year[current_year] = [hurricanes[current_cane]]
    else:
      dict_by_year[current_year].append(hurricanes[current_cane])
  
  return dict_by_year

# print(hurricanes_by_year(names, months, years, max_sustained_winds, areas_affected, deaths))
...
def count_affected_areas(names, months, years, max_sustained_winds, areas_affected, deaths):
  ...
  return d
            ...
            #or
   # without using Python's defaultdict object 

'''  count_areas_dict = {}

  for hurricane in hurricanes:
    for area in hurricanes[hurricane]["Areas Affected"]:
      if area not in count_areas_dict:
        count_areas_dict[area] = 0 
      count_areas_dict[area] +=1

  return count_areas_dict'''
  

# print(count_affected_areas(names, months, years, max_sustained_winds, areas_affected, deaths))        



# Write a function that finds the area affected by the most hurricanes, and how often it was hit
def most_affected_area(names, months, years, max_sustained_winds, areas_affected, deaths):

  areas = count_affected_areas(names, months, years, max_sustained_winds, areas_affected, deaths)

  max_area = next(iter(areas))

  max_area_count = 0

  for area in areas:
    if max_area_count < areas[max_area]:
      max_area = area
      max_area_count = areas[max_area]
 
  return max_area, max_area_count    

# print(most_affected_area(names, months, years, max_sustained_winds, areas_affected, deaths))


# Write a function that finds the hurricane that caused the greatest number of deaths, 
# and how many deaths it caused
def most_deadly_hurricane(names, months, years, max_sustained_winds, areas_affected, death):

  hurricanes = hurricane_dict(names, months, years, max_sustained_winds, areas_affected, deaths)

  max_mortality_cane = next(iter(hurricanes))

  max_mortality = 0

  for hurricane in hurricanes:
    if max_mortality < hurricanes[hurricane]['Deaths']:
        max_mortality_cane = hurricane
        max_mortality = hurricanes[hurricane]['Deaths']

  return max_mortality_cane, max_mortality 

# print(most_deadly_hurricane(names, months, years, max_sustained_winds, areas_affected, deaths))


# Write a function that rates hurricanes on a mortality scale 
def hurricane_by_mortality_scale(names, months, years, max_sustained_winds, areas_affected, deaths):
...
# print(hurricane_by_mortality_scale(names, months, years, max_sustained_winds, areas_affected, deaths))


# Write a function that finds the hurricane that caused the greatest damage, and how costly it was
def greatest_damage_hurricane(names, months, years, max_sustained_winds, areas_affected, deaths):

  hurricanes = hurricane_dict(names, months, years, max_sustained_winds, areas_affected, deaths)

  max_damage_cane = next(iter(hurricanes))
  ...
  for hurricane in hurricanes:
    if hurricanes[hurricane]['Damage'] == "Damages not recorded": 
         pass
    elif hurricanes[hurricane]['Damage'] != "Damages not recorded":  
        if max_damage < hurricanes[hurricane]['Damage']:
           max_damage_cane = hurricane
           max_damage = hurricanes[hurricane]['Damage']  

  return max_damage_cane, max_damage

# print(greatest_damage_hurricane(names, months, years, max_sustained_winds, areas_affected, deaths))


# Write a function that rates hurricanes on a damage scale 
def hurricane_by_damage_scale(names, months, years, max_sustained_winds, areas_affected, deaths):
...
# print(hurricane_by_damage_scale(names, months, years, max_sustained_winds, areas_affected, deaths))